...
const Navigation = () => {
  ...
  return (
    ...
    >
      {renderDateSelector()}
      <div>
        <button className="bg-accent-dark p-3 rounded-xl m-3 hover:scale-110 transition-all  text-white" onClick={() => handleState(new Date(), "selectedDate")}>
          Today
        </button>
        {views.length > 1 &&
          (isDesktop ? (
            views.map((v) => (
              <button
                key={v}
                className="bg-accent-dark p-3 rounded-xl m-3 hover:scale-110 transition-all text-white"
