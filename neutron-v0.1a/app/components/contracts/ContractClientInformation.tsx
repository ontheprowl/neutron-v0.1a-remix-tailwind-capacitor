...
export default function ContractClientInformation() {
    ...
    const redressalWindow = useWatch({ name: 'redressalWindow' })
    ...
    }, [startDate, endDate, redressalWindow, projectName, clientName, clientEmail, providerName, providerEmail, trigger])
    ...
    return (
        <div>
            ...
            <label htmlFor="simple-search" className="sr-only">{creator == ContractCreator.IndividualServiceProvider ? 'Service Provider Name' : 'Employer Name '}</label>
            <div className="flex flex-col sm:flex-row relative w-auto sm:items-end space-y-5 sm:space-x-10 justify-start align-middle">
                ...
                <div className=" space-y-3 w-full">
                    <span className=" prose prose-md text-white font-gilroy-regular text-[18px]">{creator == ContractCreator.IndividualServiceProvider ? 'Service Provider Name' : 'Employer Name '}</span>
                    <input type="text" id="client-name" {...formMethods.register(creator == ContractCreator.IndividualServiceProvider ? 'providerName' : 'clientName', { required: 'This field is required', maxLength: { value: 20, message: 'Client name cannot exceed 20 characters' } })} className=" bg-[#4A4A4A] pt-3 pb-3 pl-4 pr-4 border-gray-300 text-white text-sm rounded-lg placeholder-white block w-full h-10 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-white dark:text-white " placeholder="e.g : Acme Corp" required />
                    <div className="w-full h-5 mt-3 text-left">
                        <ErrorMessage errors={errors} name={creator == ContractCreator.IndividualServiceProvider ? 'providerName' : 'clientName'} render={(data) => {
                ...
                <div className=" space-y-3 w-full">
                    <span className=" prose prose-md text-white font-gilroy-regular text-[18px]">{creator == ContractCreator.IndividualServiceProvider ? 'Service Provider Email' : 'Employer Email '}</span>
                    <input type="text" id="client-email" {...formMethods.register(creator == ContractCreator.IndividualServiceProvider ? 'providerEmail' : 'clientEmail', {
                    ...
                    <div className="w-full h-5 mt-3 text-left">
                        <ErrorMessage errors={errors} name={creator == ContractCreator.IndividualServiceProvider ? 'providerEmail' : 'clientEmail'} render={(data) => {
                            return <span className="text-red-500 p-0 m-1 z-10">{data.message}</span>
                        }} />
                    </div>
                </div>
            ...
            <div className="relative w-full mt-2 mb-5 sm:mt-5 sm:mb-5 flex flex-col ">
                ...
                <div className="flex items-end mt-5 flex-col space-y-3 sm:space-x-3 sm:flex-row w-full">
                    ...
                    <div className="text-left space-y-3 w-full">
                        <span className=" prose prose-md text-white font-gilroy-regular text-[18px]">Redressal Window (in Days) </span>
                        <input {...formMethods.register('redressalWindow')} type="number" min="0" max="30" value={redressalWindow} defaultValue="14" placeholder="e.g : 15 days" className=" bg-[#4A4A4A] pt-3 pb-3 pl-4 pr-4 border-gray-300 text-white text-sm rounded-lg placeholder-white block w-full h-10 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-white dark:text-white " />
                        <div className="w-full h-5 mt-3 text-left">
