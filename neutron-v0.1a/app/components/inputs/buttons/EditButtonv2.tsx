...
export default function EditButton({ onClick, active }: { onClick?: React.MouseEventHandler<HTMLButtonElement>, active?: boolean }) {
    ...
    return (
        ...
        }} className={` ${active ? 'text-error-dark bg-error-light' : 'bg-primary-light text-primary-base '} hover:opacity-70 font-gilroy-medium text-sm transition-all min-w-fit rounded-lg p-3 space-x-2 flex flex-row items-center`}>
